class Address{
    ...
    constructor(houseno:number, street:string, area:string, zip:number, city:string, state:string, country:string){
        this.validateHouseNumber(houseno);
    ...
    validateHouseNumber(house_no: number){
        if(Number.isInteger(house_no) == false){
            throw "House Number must be an integer";
            return false;
        }
        if(house_no <= 0){
            throw "House number must be positive";
        }
        if(house_no >= 990000){
            throw "house number can't be this large";
        }
    }
    validateZipCode(zip_code: number){
        ...
        if( !(zip_code/1000000 == 0 && zip_code/1000000 != 0)){
            throw "Zip Code must have six numbers";
        }
    } 
    display(){
    ...
    set_house_no(new_house_no: number){
        this.validateHouseNumber(new_house_no);
...
class ContactDetails extends Address{
    ...
    constructor(phone_no: number, mail: string, houseno: number, street:string, area:string, zip:number, city:string, state:string, country:string){
        ...
        this.validateNumber(phone_no)
        this.phone_no = phone_no
        this.validateEmailId(mail)
        this.email_id = mail
    }
    display(){
    ...
    validateNumber(phone_no: number){
        if(Number.isInteger(phone_no) == false){
            throw "Phone Number must consist of integers";
        }
        if(phone_no <= 0){
            throw "Phone number must be positive"
        }
        if( !(phone_no/100000000000 == 0 && phone_no/10000000000 != 0)){
            throw "Phone Number must have 10 numbers";
        }
    }
    validateEmailId(mail: string){
        ...
        if(valid == false){
            throw "Not an acceptable mail "
        }
        // return valid;
    }
    set setPhoneNo(new_phone_no: number){
        this.validateNumber(new_phone_no)
        this.phone_no = new_phone_no
    }
    set setMail(new_mail: string){
        this.validateEmailId(new_mail) 
