...
class Book{
    ...
    addStock(no_of_stock: number = 1){
        for(var i=0;i<no_of_stock;i++){
            this.stocks.push({memberId:null, rentDate : null})
        }
    }
    rentBookStock(stock_id: number, member_id: number, rentDate: object){
    ...
    requestRent(member_id: number, rentDate: object){
    ...
    returnBook(stock_id: number): any{
...
var creatingAbundance = new Book("Creating Abundance", "Jeet")

console.log("creatingAbundance.addStock(1)")
creatingAbundance.addStock(1)
creatingAbundance.display()
...
class Library{
    ...
    addStock(book_id: number, no_of_stock: number = 1){
    ...
    rentBookStock(book_id: number, stock_id: number, member_id: number, rentDate: object){
        if(book_id >= this.books.length){
            console.log("No Such Book registered")
        }
        else this.books[book_id].rentBookStock(stock_id, member_id, rentDate)
    }
    requestRent(book_id: number, member_id: number, rentDate: object){
        if(book_id >= this.books.length){
            console.log("No Such Book registered")
        }
        else this.books[book_id].requestRent(member_id, rentDate)
    }
    returnBook(book_id: number, stock_id: number, returnDate: object){
